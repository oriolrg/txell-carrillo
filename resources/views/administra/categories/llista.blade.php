  ...
  <tbody>
    @isset($categories)
      ...
      <tr>
        ...
        <td>{{$data->nom}}</td>
        <td>
          <a class="btn btn-small btn-info" href="{{ URL::to('administra/categories/' .$data->id. '/edit') }}">Edita</a>
          <!--<a class="btn btn-small btn-warning" href="{{ URL::to('administra/categories/' .$data->id. '/desactiva') }}">Desactiva</a>-->
          <form action="{{ URL::to('administra/categories/'.$data->id ) }}/delete" method="POST">
            {{ method_field('DELETE') }}
            @csrf
